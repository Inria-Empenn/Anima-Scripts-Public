...
animaScriptBrainDir = configParser.get("anima-scripts",'anima-script-brain-extract')

# Argument parsing

parser = argparse.ArgumentParser(description="Compute cerebral blood flow (CBF) from ASL, M0 and T1 images")
parser.add_argument('-asl', '--asl', type=str, default = os.getcwd()+"/n019/perf/ASL_4D.nii", help="ASL 4D file")
parser.add_argument('-m0', '--m0', type=str, default = os.getcwd()+"/n019/M0/M0_1.nii", help="M0 file")
parser.add_argument('-t1', '--t1', type=str, default = os.getcwd()+"/n019/anat/T13D_n019.nii.gz" , help='T13D file')
parser.add_argument('-o', '--o', type=str, default = os.getcwd(), help='output directory')
...
parser.add_argument('-firstVolLabel', '--firstVolLabel', type=int, default=0, help='1=label or 0=control')
...
parser.add_argument('-template', '--template', type=str, default=os.getcwd()+"/averageForm9.nii.gz", help='Template file')
parser.add_argument('-mask', '--mask', type=str, default="", help='Mask file (based on T1 image)')

# Quantif arguments

parser.add_argument('-tau', '--tau', type=float, default= 1509, required=False, help='label duration (ms)')
parser.add_argument('-sDelay', '--sDelay', type=float, default= 37.5, required=False, help='slice delay (ms)')
parser.add_argument('-alpha', '--alpha', type=float, default= 0.85, required=False, help='labeling efficiency')
parser.add_argument('-lambdaa', '--lambdaa', type=float, default= 0.9, required=False, help='blood Partition Coefficient')
parser.add_argument('-PLD', '--PLD', type=float, default= 1350, required=False, help='post labelling delay (ms)')
parser.add_argument('-T1Blood', '--T1Blood', type=float, default= 1650, required=False, help='longitudinal relaxation time of blood (ms)')
...
MaskT1ImageCommand= [animaScriptBrainDir + "animaAtlasBasedBrainExtraction.py", "-i", T1Image, "-b", outT1Prefix+ "_masked.nii.gz"]
call(MaskT1ImageCommand, stdout=open(os.devnull, "w"))
MaskImage=outT1Prefix + "_masked"
...
print("----------Mean m0 ---------")
listM0File = open("M0_list.txt", 'w')
matches = []
for root, dirnames, filenames in os.walk(os.getcwd()):
    for filename in fnmatch.filter(filenames, 'M0_*.nii*'):
        matches.append(os.path.join(root, filename))
        print(os.path.join(root, filename), file= listM0File)
listM0File.close()
M0MeanImageCommand= [animaDir + "animaAverageImages", "-i", outDir + "/M0_list.txt", "-o", outDir + "/M0_mean.nii.gz"]
call(M0MeanImageCommand, stdout=open(os.devnull, "w"))
M0Image= "M0_mean.nii.gz"
...
if args.template != "":
    ...
    print("diffeomorphic")
    print("animaDenseSVFBMRegistration")
    diffeoTemplateCommand=[animaDir + "animaDenseSVFBMRegistration", "--sym-reg", str(1), "--es", str(3), "--fs", str(2), "-m", outT1Prefix + "_regTemplate_aff.nii.gz", "-r", args.template, "-o", outT1Prefix + "_regTemplate_diffeo.nii.gz","-O", outReg + "_regTemplate_diffeo_tr.nii.gz" ]
    call(diffeoTemplateCommand, stdout=open(os.devnull, "w"))

    print("animaTransformSerieXmlGenerator")
    ...
    call(transfoGenCommand, stdout=open(os.devnull, "w"))
    print("animaApplyTransformSerie")
    applyTransfoCommand=[animaDir + "animaApplyTransformSerie", "-i", outASLPrefix + "_CBF_masked.nii.gz", "-g", args.template, "-t", outReg + "_regTemplate_tr.xml", "-o", outASLPrefix + "_CBF_regTemplate.nii.gz" ]
    call(applyTransfoCommand, stdout=open(os.devnull, "w"))
